...
export interface IEventoApi {
  ...
  adicionarVoluntarioEvento: (
    voluntario: InformacoesUsuario,
    evento: DadosEvento
  ) => Promise<DadosEvento>;
  adicionarVoluntarioTurno: (
    voluntario: InformacoesUsuario,
    evento: DadosEvento,
...
export const EventoApiMock: IEventoApi = {
  ...
  adicionarVoluntarioEvento: async (
    voluntario: InformacoesUsuario,
    evento: DadosEvento
  ): Promise<DadosEvento> => {
    console.log(
      'Mock - Adicionando voluntario ao evento:',
      voluntario,
      'no evento',
      evento
    );
  ...
  adicionarVoluntarioTurno: async (
    voluntario: InformacoesUsuario,
    evento: DadosEvento,
    diaEvento: DiaDeEvento,
    turno: Turno
    ...
    console.log(
      'Mock - Adicionando voluntario ao turno:',
      voluntario,
      'no evento',
      evento,
      'no dia',
    ...
    const diaIndex = evento.dias.findIndex(
      (dia) => dia.data === diaEvento.data
    );
    const turnoIndex = evento.dias[diaIndex]?.turnos.findIndex(
    ...
    if (diaIndex !== -1 && turnoIndex !== -1) {
      ...
      if (
        !evento.dias[diaIndex].turnos[turnoIndex].voluntarios.some(
